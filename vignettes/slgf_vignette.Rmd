...
That is, we compute $\hat{\sigma}^2_{\text{1,2,3}}=$ `r round(as.numeric(smell.hats[5]), 5)` and $\hat{\sigma}^2_{\text{4,5}}=$ `r round(as.numeric(smell.hats[4]), 5)`. Let us also consider the case where ``het=c(1,1,1)``; that is, we include two additional classes: group-based variances and a single global mean, and group-based variances with means by ``agecat``. 
...
```{r, cache=FALSE, echo=TRUE, message=FALSE, results='hide', warning=FALSE}
